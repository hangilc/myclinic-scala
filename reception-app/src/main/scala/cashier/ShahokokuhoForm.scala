...
case class ShahokokuhoForm(init: Option[Shahokokuho]):
  ...
  val (hokenshaBangouProp, hihokenshaKigouProp, hihokenshaBangouProp, edabanProp, honninProp, koureiProp, validFromProp, validUptoProp)
    = props

  val validateProps = (
    hokenshaBangouProp, hihokenshaKigouProp, hihokenshaBangouProp, honninProp, validFromProp, validUptoProp, koureiProp, edabanProp
  )
  ...
  def validateForEnter(patientId: Int): Either[String, Shahokokuho] =
    val rs = Prop.resultsOf(validateProps)
    ShahokokuhoValidator.validate(
  ...
  def validateForUpdate: Either[String, Shahokokuho] =
    val rs = Prop.resultsOf(validateProps)
    ShahokokuhoValidator.validate(
      ShahokokuhoIdValidator.validateOptionForUpdate(init.map(_.shahokokuhoId)) 
