...
class Cashier(
  ...
  channels: ReceptionSubscriberChannels
) extends SideMenuService:
  val unsubs = List(
    channels.wqueueCreatedChannel.subscribe(onWqueueCreated),
    channels.wqueueDeletedChannel.subscribe(onWqueueDeleted)
  )
  ...
  val ele: HTMLElement = div(
    ...
    div(
      ...
      button(
        "新規患者",
        cls := "reception-cashier-new-patient-button",
        onclick := (doNewPatient _)
      ),
      recordsMenu.ele,
      Icons.menu(
