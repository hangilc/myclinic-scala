...
case class WqueueRow(
    var wqueue: Wqueue,
    visit: Visit,
    var patient: Patient
)(using
    dataId: DataId[Wqueue],
    moelSymbol: ModelSymbol[Wqueue],
  ...
  val unsubs = List(
    channels.wqueueUpdatedChannel.subscribe(onWqueueUpdated),
    channels.patientUpdatedChannel.subscribe(onPatientUpdated)
  )
