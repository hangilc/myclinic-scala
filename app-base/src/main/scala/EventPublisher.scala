...
class EventPublishers:
  ...
  def publish(event: AppModelEvent): Unit =
    ...
    (event.model, event.kind) match {
      ...
      case (Charge.modelSymbol, C) => onChargeCreated(event.data.asInstanceOf[Charge])
      case (Charge.modelSymbol, U) => 
        println(("charge", event.data))
        onChargeUpdated(event.data.asInstanceOf[Charge])
      case (Charge.modelSymbol, D) => onChargeDeleted(event.data.asInstanceOf[Charge])
  ...
  def onChargeCreated(charge: Charge): Unit = ()
  def onChargeUpdated(charge: Charge): Unit = ()
  def onChargeDeleted(charge: Charge): Unit = ()
